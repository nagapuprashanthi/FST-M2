-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/praha/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, lines:chararray);
inputDialogues5 = LOAD 'hdfs:///user/praha/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, lines:chararray);
inputDialogues6 = LOAD 'hdfs:///user/praha/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, lines:chararray);
...
rmf hdfs:///user/praha/outputs;

-- Store result in HDFS
STORE namesOrdered INTO ' hdfs:///user/praha/outputs' USING PigStorage('\t');
